...
document.getElementById('submit-data-btn').onclick = async () => {
    ...
    try {
        ...
        render(true); // After submit, show only what exists in DB
...
// Initial Load: Show today's data immediately
render(true);

document.getElementById('logout-btn').onclick = () => signOut(auth);

// CLEAR BUTTON: Wipes all input values but keeps the rows
document.getElementById('clear-btn').onclick = () => {
    if(confirm("Clear all data from this view?")) {
        render(false); // Render rows with no data fetch
    }
};